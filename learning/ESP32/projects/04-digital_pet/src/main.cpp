...
//stats
enum class Mood { HAPPY, NEUTRAL, SAD, SLEEPY, EXCITED, EATING };
...
public:
    ...
    void feed()  { hunger_ = clamp(hunger_ + 25);      react(Mood::EATING, 1500); touch(); }
    void play()  { happiness_ = clamp(happiness_ + 20); energy_ = clamp(energy_ - 10); react(Mood::EXCITED, 1500); touch(); }
    void clean() { cleanliness_ = clamp(cleanliness_ + 30); touch(); }
...
void drawFace(Mood m) {
    ...
    switch (m) {
        case Mood::HAPPY:
            // curved "^ ^" eyes
            display.drawLine(leftX - 8, eyeY, leftX, eyeY - 8, SSD1306_WHITE);
            display.drawLine(leftX, eyeY - 8, leftX + 8, eyeY, SSD1306_WHITE);
            display.drawLine(rightX - 8, eyeY, rightX, eyeY - 8, SSD1306_WHITE);
            display.drawLine(rightX, eyeY - 8, rightX + 8, eyeY, SSD1306_WHITE);
            display.drawCircle(64, 40, 14, SSD1306_WHITE); // big smile (bottom half feel)
            break;
        case Mood::EXCITED:
        ...
        case Mood::EATING:
            display.fillCircle(leftX, eyeY, eyeR, SSD1306_WHITE);
            display.fillCircle(rightX, eyeY, eyeR, SSD1306_WHITE);
            display.fillCircle(64, 44, 6, SSD1306_WHITE); // open "om nom" mouth
            break;
        case Mood::SAD:
            display.drawCircle(leftX, eyeY, eyeR, SSD1306_WHITE);
            display.drawCircle(rightX, eyeY, eyeR, SSD1306_WHITE);
            display.drawLine(56, 46, 72, 40, SSD1306_WHITE); // frown
            break;
        case Mood::SLEEPY:
...
void setLedsForStats() {
    constexpr uint8_t kLowThreshold = 30;
